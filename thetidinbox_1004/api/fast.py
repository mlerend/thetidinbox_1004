...
def all_functions(test):
    spam = pred_spam(pd.DataFrame({"message": test})).tobytes()
    pro_perso = pred_pro_perso(pd.DataFrame({"body": test})).tobytes()
    urgent_class = urgent_categories(pd.DataFrame({"body": test}))['present']
    urgent_class = np.array(urgent_class.apply(lambda x : 1 if x else 0)).tobytes()
    action_class = action_categories(pd.DataFrame({"body": test}))
    unique_action_class = np.array(unique_values(action_class['columnspresent'])).tobytes()
    meeting = np.array(pred_meeting_invit(test)).tobytes()
    topic = np.array(pred_bertopic(pd.DataFrame({"body": test}))).tobytes()
